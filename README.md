...
- Rollar və autentifikasiya: `resident`, `komendant`, `superadmin`
...
- `ENABLE_INIT_ROUTE` - demo məqsədli `/init` marşrutunu aktiv edir
...
- bazada `superadmin` yoxdursa bootstrap superadmin yaradır
...
İşə salmazdan əvvəl `SECRET_KEY` ötürülməlidir, məsələn `.env` vasitəsilə.

## Standart giriş məlumatları

### Avtomatik bootstrap superadmin
...
### `/init` ilə demo məlumatlar

`/init` marşrutu standart olaraq söndürülüb və yalnız bu halda işləyir:

- `ENABLE_INIT_ROUTE=1` təyin olunub
- baza hələ boşdur

Bu marşrut demo məlumatlar yaradır:

- `admin@emtk.itg.az` / `admin`
- `commandant@smartzhk.local` / `commandant123`
- `resident@smartzhk.local` / `resident123`
- test mənzili, tariflər, elan və worklog qeydləri
